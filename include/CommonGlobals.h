...
inline json deflation2xMenuNav = {
        {"mapDifficulty", "expert"}, {"map", "infernal"}, {"difficulty", "easy"}, {"mode", "deflation"}
};

// Helper function to replace all occurrences of a character
inline string replaceChar(std::string str, char oldChar, char newChar) {
    std::replace(str.begin(), str.end(), oldChar, newChar);
    return str;
}

// Helper function to convert string to title case
inline string toTitleCase(std::string str) {
...
inline string removeSubstring(std::string str, const std::string &toRemove) {
...
inline int genData(){
    if(!exists("Tower Positions")){
        create_directory("Tower Positions");
    }
    if(!exists("Tower Positions/deflation.json")) {
        CoordinateHandler::gen(deflationTowData, deflationMenuNav, "deflation");
    }
    if(!exists("Tower Positions/deflation_2x_cash.json")){
